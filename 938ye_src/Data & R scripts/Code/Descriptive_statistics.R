...
# The "intentional" group from the manuscript is referred to as "explicit learningtype" in this R script.
...
# Calculate difference scores (T2 minus T1)
participantsdata$crit_input_DIFF <- participantsdata$crit_input_T2 - participantsdata$crit_input_T1
participantsdata$crit_noinput_DIFF <- participantsdata$crit_noinput_T2 - participantsdata$crit_noinput_T1
participantsdata$contr_input_DIFF <- participantsdata$contr_input_T2 - participantsdata$contr_input_T1
participantsdata$contr_noinput_DIFF <- participantsdata$contr_noinput_T2 - participantsdata$contr_noinput_T1
...
scores.all <- basicStats(participantsdata[, 28:39])
setDT(scores.all, keep.rownames = TRUE)
colnames(scores.all)[1] <- "descriptives"
scores.exp <- basicStats(pp.explicit[, 28:39])
setDT(scores.exp, keep.rownames = TRUE)
colnames(scores.exp)[1] <- "descriptives"
scores.inc <- basicStats(pp.incidental[, 28:39])
setDT(scores.inc, keep.rownames = TRUE)
colnames(scores.inc)[1] <- "descriptives"
scores.una <- basicStats(pp.unaware[, 28:39])
setDT(scores.una, keep.rownames = TRUE)
colnames(scores.una)[1] <- "descriptives"
scores.all2 <- basicStats(pp.explicit.incidental[, 28:39])
setDT(scores.all2, keep.rownames = TRUE)
colnames(scores.all2)[1] <- "descriptives"
...
## Get descriptives for the difference scores (T2 - T1)
library(Rmisc)
summarySE(participantsdata, measurevar = "crit_input_DIFF", groupvars = c("learningtype"))

b <- boot(pp.explicit$crit_input_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

b <- boot(pp.incidental$crit_input_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

summarySE(participantsdata, measurevar = "crit_noinput_DIFF", groupvars = c("learningtype"))

b <- boot(pp.explicit$crit_noinput_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

b <- boot(pp.incidental$crit_noinput_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

summarySE(participantsdata, measurevar = "contr_input_DIFF", groupvars = c("learningtype"))

b <- boot(pp.explicit$contr_input_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

b <- boot(pp.incidental$contr_input_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

summarySE(participantsdata, measurevar = "contr_noinput_DIFF", groupvars = c("learningtype"))

b <- boot(pp.explicit$contr_noinput_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)

b <- boot(pp.incidental$contr_noinput_DIFF, samplemean, R = 10000)
plot(b)
boot.ci(b)
...
participantsdata.long$verbtype <- factor(participantsdata.long$verbtype)
is.factor(participantsdata.long$verbtype)
participantsdata.long$input <- factor(participantsdata.long$input)
is.factor(participantsdata.long$input)
participantsdata.long$testmoment <- factor(participantsdata.long$testmoment)
is.factor(participantsdata.long$testmoment)
...
interaction2wt(score ~ learningtype * verbtype * input * testmoment,
  ...
  box.ratio = .3, rot = c(90, 90), factor.expressions =
    c(
      Condition = expression("Group"),
      Type = expression("Verb Type"),
      Input = expression("Input"),
      Moment = expression("Test Moment")
    ), responselab.expression = "percentage"
)
